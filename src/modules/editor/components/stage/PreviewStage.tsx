...
export function PreviewStage() {
  ...
  const setWasTabHiddenPaused = usePlaybackStore(
    (state) => state.setWasTabHiddenPaused,
  );
      ...
      {wasTabHiddenPaused && (
        <div className="flex items-center justify-between bg-studio-panel-raised border-b border-studio-border px-4 py-1.5 text-xs text-selection">
          <span className="flex items-center gap-1.5">
            <AlertCircle className="h-3.5 w-3.5" /> Playback paused
            automatically because tab was hidden.
          </span>
          <button
            type="button"
