...
export function CapabilitiesSection() {
  ...
  return (
    <section id="capabilities" className="py-20 bg-[#F6F4EF] text-[#151619] border-t border-[#D9D5CC]">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {capabilities.map((cap) => {
            ...
            return (
              <div
                key={cap.title}
                className="rounded-2xl border border-[#D9D5CC] bg-[#FFFFFF] p-6 shadow-xs hover:border-[#FF5A36]/50 transition-colors"
              >
                <div className="flex h-9 w-9 items-center justify-center rounded-xl bg-[#FFF0EB] text-[#FF5A36] mb-4">
                  <Icon className="h-4 w-4" />
                </div>
                <h3 className="text-sm font-bold text-[#151619] mb-1.5">{cap.title}</h3>
                <p className="text-xs text-[#6F716F] leading-relaxed">{cap.description}</p>
              </div>
            );
          })}
        </div>
